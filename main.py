...
def generate_code_with_llm(brief: str, checks: List[str]) -> str:
    prompt = (
        "Create a complete, single-page HTML application.\n\n"
        "REQUIREMENTS:\n"
        f"{brief}\n\n"
...
def generate_readme(task: str, brief: str, checks: List[str]) -> str:
    prompt = (
        f"Create a professional README.md for a project named '{task}'.\n\nDESCRIPTION:\n{brief}\n\n"
...
def generate_fallback_html(brief: str, checks: List[str]) -> str:
    return (
        ...
        f"<h1>{brief}</h1><ul>{''.join(f'<li>{c}</li>' for c in checks)}</ul>"
        "<p>AI generation failed.</p></body></html>"
    )
...
def git_workflow(repo_name: str, files: Dict[str, str]) -> tuple:
    ...
    subprocess.run(["git", "config", "user.name", GITHUB_USERNAME], check=True, shell=True)
    subprocess.run(["git", "config", "user.email", f"{GITHUB_USERNAME}@users.noreply.github.com"], check=True, shell=True)
    for filename, content in files.items():
        with open(filename, "w", encoding="utf-8") as f: f.write(content)
    subprocess.run(["git", "add", "."], check=True, shell=True)
    subprocess.run(["git", "commit", "-m", "Update for round {round}".format(round=files.get('round', 1))], check=True, shell=True)
...
def process_task(task_request: TaskRequest):
    try:
        print(f"\n🎯 Processing Task: {task_request.task} (Round {task_request.round})")
        ...
        eval_response = EvaluationResponse(
            repo_url=repo_url, commit_sha=commit_sha, pages_url=pages_url, **task_request.model_dump()
        )
        submit_evaluation(task_request.evaluation_url, eval_response)
        print(f"\n✅ Task '{task_request.task}' (Round {task_request.round}) completed successfully!")
    except Exception as e:
